...
impl Postfix {
    pub fn new(operation: &str) -> Result<Self, Err> {
        let tokens = match Postfix::tokenize(operation) {
            Ok(t) => t,
            Err(err) => return Err(err),
        };

        let postfix = match Postfix::postfixer(tokens) {
            Ok(p) => p,
            Err(err) => return Err(err),
        };

        Ok(Postfix { pf: postfix })
    }

    pub fn as_string(&self) -> String {
        let mut iter = self.pf.iter();
    ...
    pub fn calculate(&self) -> f64 {
        let result = Postfix::evaluate(&self.pf).unwrap();
        return result;
    }
    ...
    fn postfixer(arg: Vec<Token>) -> Result<Vec<Token>, Err> {
        ...
        while let Some(&i) = arg_iter.next() {
            match i {
                ...
                Token::Operator(op) => {
                    ...
                    let current_precedence = Postfix::precedence(&i).unwrap();
                    let last_token_precedence = Postfix::precedence(&last_token_in_stack).unwrap();
        ...
        while let Some(n) = stack.pop() {
            if let Token::Operator(OperatorType::StartCaret) = n {
                continue;
            }
            res.push(n);
        }

        if let Err(err) = Postfix::evaluate(&res) {
            return Err(err);
        }
    ...
    fn tokenize(arg: &str) -> Result<Vec<Token>, Err> {
        let str = Postfix::sanitize_whitespace(arg);
